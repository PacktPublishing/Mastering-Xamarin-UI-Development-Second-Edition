    ...
    public partial class WalksMainPage : ContentPage
    {
        ...
        public async void OnDeleteItem(object sender, EventArgs e)
        {
            ...
            if (await DisplayAlert("Delete Walk Entry Item",
                                   ...
                                   "OK", "Cancel"))
            {
                // Remove Walk Item from our WalkListModel collection and SQL Server database
                _viewModel.WalksListModel.Remove(selectedItem);

                await _viewModel.AzureDatabase.DeleteWalkEntry(selectedItem.Id);
                await DisplayAlert("Delete Walk Entry Item",
                                   selectedItem.Title +
